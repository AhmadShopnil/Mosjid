...
export default function CardCurriculum({ curriculum, curriculumNo = 1 }) {
            ...
            <div className="w-full h-[221px] flex justify-end relative border-8 border-l-[#FFCE4D] border-t-[#FFCE4D] border-b-white border-r-white rounded-tl-[60px]">
              <div className="absolute w-[98%] top-8">
                <div className="flex flex-wrap gap-2">
                  {
                    curriculum?.item_lists?.map((data, i) => <Card
                      key={i}
                      data={data}
                      onViewDetails={() => openModal('en')} onViewJapanese={() => openModal('ja')}
                    />)
                  }
        ...
        <div className="space-y-6 ">
          {
            curriculum?.item_lists?.map((data, i) => <Card
              key={i}
              data={data}
              title="Academic Focus" onViewDetails={() => openModal('en')} onViewJapanese={() => openModal('ja')}
            />)
          }
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-[30px] p-6 md:p-10 w-full max-w-3xl max-h-[90vh] overflow-y-auto relative shadow-2xl border-4 border-[#FFCE4D]">
            ...
            <div className="text-center mb-8 border-b-2 border-gray-100 pb-8">
              <span className="text-[50px] font-bold text-[#B98C20] block mb-2"> {curriculumNo}</span>
              <h2 className="text-[32px] font-bold text-[#005312] leading-tight mb-2">{curriculum?.title}</h2>
              <p className="text-[20px] font-medium text-[#B98C20]">{curriculum?.short_description}</p>
            </div>
...
function Card({ data, showButtons = true, }) {


  const onViewDetailsModal = (data) => {
    setModalData(data)
  }
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      >
        ...
        >
          <span className='text-[15px] lg:text-[18px]'> {data?.title?.en}</span>
        </div>
          ...
          <ul className="space-y-2">
            {data?.items.map((item, index) => (
              <li key={index} className="flex items-center gap-3 border-b border-b-[#E0E0E0] pb-3 last:border-b-0 ">
                ...
                <span className="text-[#333333] text-[15px] lg:text-[18px] leading-snug">{item?.en}</span>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div className="flex gap-2 bg-white p-3 border-t-2 border-[#C9E9BA] shadow-xl rounded-bl-[20px] rounded-br-[20px]">
          ...
          <div className="relative flex-1">
            <button
              ...
              onClick={() => onViewDetailsModal(data?.description?.en)}
          ...
          <div className="relative flex-1">
            <button
              ...
              onClick={() => onViewDetailsModal(data?.description?.jp)}
